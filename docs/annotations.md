...
| `stubby.io/auto-rescue` | `true` \| `false` | `false` | Experimental. Requires the controller (`controller.enabled`). When the container is stuck in `ImagePullBackOff`/`ErrImagePull`, swap it to a dummy in place and revert once the real image is available. The controller defaults to the backend dummy; `stubby.io/type: frontend` selects the frontend but only makes sense where the webhook won't stub the pod proactively (see the README caveat). |
